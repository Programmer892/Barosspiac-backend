...
//Get conversaitons for the user
...
async function postConversation(req, res) {
  ...
  try {
    const [exist] = await pool.query(
      `SELECT *,users.fullname FROM conversations INNER JOIN users ON users.user_id = ? WHERE (user1_id = ? AND user2_id = ?) OR (user2_id = ? AND user1_id = ?)`,
      [user2_id, user1_id, user2_id, user1_id, user2_id],
    );
    console.log(exist);
    if (exist.length > 0) {
