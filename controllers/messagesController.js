...
//Get unreaded message count  for a conversation
  ...
  try {
    const [result] = await pool.query(
      `UPDATE messages
             ...
             AND sender_id != ?
             AND read_at IS NULL`,
      [conversations_id, user_id],
    );

    io.to(String(conversations_id)).emit("messages_read", {
      conversation_id: Number(conversations_id),
    });
    return res
      .status(200)
      .json({ message: `${result.affectedRows} üzenet olvasottként jelölve` });
  } catch (error) {
    console.log(error);
    return res
      .status(500)
