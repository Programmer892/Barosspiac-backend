...
//Get  ratings statistics by user_id
async function getRatingsById(req, res) {
  ...
  try {
    const [result] = await pool.query(
      `SELECT AVG(rate) AS "avg",COUNT(*) AS "db" FROM ratings WHERE rated_id = ?`,
      [rated_id],
    );
...
//Get all comment / rating card by user
async function getRatingsByUser(req, res) {
  ...
  try {
    const [result] = await pool.query(
      "SELECT ratings.*,users.fullname,users.pfp FROM ratings INNER JOIN users ON users.user_id = ratings.rater_id WHERE rated_id = ? ORDER BY created_at DESC ",
      [rated_id],
    );
...
async function deleteRating(req, res) {
  ...
  try {
    const [result] = await pool.query(
      "DELETE FROM ratings WHERE `ratings`.`rating_id` = ?",
      [rating_id],
    );
...
async function updateRating(req, res) {
  ...
  try {
    const [result] = await pool.query(
      "UPDATE `ratings` SET  `rate` = COALESCE(NULLIF(?, ''), rate) , `text` = COALESCE(NULLIF(?, ''), text) WHERE `ratings`.`rating_id` = ?",
      [rate, text, rating_id],
    );
